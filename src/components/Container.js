                ...
                <div className="container-lg mt-3 border" style={{backgroundColor:"#fcd0c7", height:"550px", borderRadius:"20px", boxShadow:"2px 2px 8px black"}}>
                    <div className='d-flex justify-content-center align-items-center my-5'>
                        {(this.state.toggle)?arr.map((item) => {
                            return(
                                <button onClick={this.checkColor} className='mx-2 border-0 shadow-lg' style={{width:"50px", height:"50px", backgroundColor:item, boxShadow:"2px 2px 10px #3b383a"}} ></button>
                            );
                        })
                        :<p></p>}
                    </div>
                    <div className='d-flex justify-content-center align-items-center'>
                        <button id="clickButton" className='text-light border-0 px-3 py-2' onClick={this.handleColor} style={{backgroundColor:this.state.color,boxShadow:"2px 2px 8px black"}}>Pick a color</button>
                    </div>
                </div>
            </div>
        );
